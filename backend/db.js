...
async function connectDB() {
    ...
    const usingCustomUri = hasCustomMongoUri();
    const canFallbackToMemory = !usingCustomUri || allowInMemoryFallback();
    ...
    try {
        ...
        if (!canFallbackToMemory) {
            printMongoConnectionHelp(error, uri);
            throw error;
        }

        console.log('Real MongoDB not found. Starting in-memory MongoDB...');
    }
...
async function autoSeed() {
    ...
    for (const book of BOOKS) await Book.create({ ...book, available: book.copies });
