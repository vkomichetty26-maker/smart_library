...
const { getMongoUri, printMongoConnectionHelp } = require('./mongoConfig');
...
async function seed() {
    try {
        ...
        for (const book of BOOKS) {
            await Book.create({ ...book, available: book.copies });
            process.stdout.write('.');
        }
        ...
        await Settings.create({
            institution: 'SmartLib University',
            finePerDay: 5,
            maxBooksStudent: 3,
            maxBooksFaculty: 10,
            daysStudent: 14,
        ...
        printMongoConnectionHelp(error, MONGO_URI);
