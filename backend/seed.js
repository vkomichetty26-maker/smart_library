...
const MONGO_URI = process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/library_db';
...
const BOOKS = [
    ...
    { title:'Database System Concepts',          author:'Silberschatz, Korth, Sudarshan',isbn:'978-0078022159', category:'Databases',          publisher:'McGraw-Hill',     edition:'7th', year:2019, copies:5 },
...
async function seed() {
    try {
        ...
        // Clear existing (except users if you want to preserve logins)
        await User.deleteMany({});
        await Book.deleteMany({});
        await Settings.deleteMany({});

        console.log('👤 Creating users…');
        for (const u of USERS) { await User.create(u); process.stdout.write('.'); }
        console.log(`\n✅ Created ${USERS.length} users`);

        console.log('\n📚 Creating books…');
        for (const b of BOOKS) {
            await Book.create({ ...b, available: b.copies });
